...
import { getCards, getIdGame } from '../helpers/cardsHelper';
...
export const GameProvider = ({ children }) => {
    ...
    const playGame = async () => {
        setIdGame(await getIdGame());
    }

    const setInitialGame = async () => {
        setShow(false)
        let cards = await getCards(idGame, 10);
        cards = await changeLettersOfNumber(cards);
        fillGame(cards, setPlayer1, player1);
        cards = await getCards(idGame, 10);
        cards = await changeLettersOfNumber(cards);
        fillGame(cards, setPlayer2, player2);
    };

    const requestCards = async () => {
        let cards = await getCards(idGame, 2);
        cards = await changeLettersOfNumber(cards);

        let completed = player1.completed;
        const player1Status =
            completed.cuarta && completed.terna && completed.terna2;
        completed = player2.completed;
        const player2Status =
            completed.cuarta && completed.terna && completed.terna2;

        if (player1Status || player2Status) {
            setShowToast(true);
            setWinName(player1Status ? player1.name : player2.name);
            return;
        }
        if (cards.length === 0) {
            setShowToast(true);
        ...
        //setPlayer1({ ...player1, cards: [...player1.cards, cards[0]].sort(compare) })
        //setPlayer2({ ...player2, cards: [...player2.cards, cards[1]].sort(compare) })
        changeCard(cards[0], player1, setPlayer1);
        changeCard(cards[1], player2, setPlayer2);
        setObtainedCards(cards);

    }

    const changeLettersOfNumber = async cards => {
        cards = cards.map(c => {
            c.value === 'ACE' ? '1' : c.value;
            c.value === 'JACK' ? '11' : c.value;
            c.value === 'QUEEN' ? '12' : c.value;
            c.value === 'KING' ? '13' : c.value;
            return c;
        });
        return cards;
    };

    const fillGame = (cards, setPlayer, player) => {
        let playerCards = {
            terna: [],
            terna2: [],
            cuarta: [],
    ...
    return (
        <GameContext.Provider value={{
            requestCards, playGame, player1, setPlayer1, player2, setPlayer2, compare, obtainedCards, setInitialGame, show, setValidate, showToast,
            setShowToast, winName
        }}>
            {children}
